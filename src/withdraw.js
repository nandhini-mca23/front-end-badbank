...
//     var [total,setTotal] = useState(ctx.user.find(user=>(user.name === name && user.password === pass)?user.amount:null));
...
//     <button onClick={handleClick} disabled={name === '' || pass === '' ||withdraw<=0 || ctx.user.find(user=>(user.name === name && user.password === pass)?user.amount<onamt:null)}>Withdraw</button>
...
function Withdraw(){
    ...
    useEffect(()=>{
        async function fetchData(){
            if(name&&pass){
                await axios.get('https://server-1-tq16.onrender.com/data').then(res=>{console.log(res.data);
                setData(res.data);
    ...
    async function handleClick(e){
        ...
        for(let i=0; i<data.length; i++){
            if(data[i].name === name && data[i].password === pass){
                await axios.put(`https://server-1-tq16.onrender.com/update/${data[i]._id}`,{amount:data[i].amount - Number(withdraw)});
...
<div className="create">
    <Card className='card' border="success" style={{ width: '35rem',height:'27rem' }}>
        <Card.Header className='account'>Withdraw</Card.Header>
        <Card.Body>
        <form>
        <p className="total">Available Balance &nbsp;Rs. {total}</p>
